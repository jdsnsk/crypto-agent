### Task 5: Vercel Serverless AI 分析函数

**Files:**
- Create: `api/analyze.js`

**Interfaces:**
- POST `/api/analyze` — 接收行情数据 + 用户问题，调 Claude API 返回分析

- [ ] **Step 1: 编写 api/analyze.js**
...
git commit -m "feat: Vercel Serverless AI 分析代理（Claude API）"
```
...
1. **Subagent-Driven（推荐）** — 每个 Task 派独立子 agent 执行，我在任务间审查代码
2. **顺序执行** — 在当前会话中逐个编写，边写边确认
